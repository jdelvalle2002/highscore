...
def clickOn(x,y):
    global rounds, buttons, colors, rows, cols, valor, Board, ronda, ronda_t, el_dado
    ...
    if rounds == 26:
        ...
        #ronda_t = "Juego finalizado"
        line = ""
        for lines in Board.msj:
            line += lines + "\n"
        tkinter.messagebox.showinfo("Gave Over", f"{line} Puntaje final: {puntos_t}\nPulse Restart para volver a jugar")
...
def restartGame():
    ...
    rounds = 1
    ronda_t = f"Ronda {rounds}"
    ronda = tkinter.Label(window, text= ronda_t, fg='black', font=("Courier", 20, "bold"), justify= LEFT) #como hacer que desaparezca un mensaje???
    ronda.pack()
    ronda.place(x=160,y=400)           
    valor = dado()
    el_dado = tkinter.Label(window, text= f"Dado: {valor}", fg='black', font=("Courier", 20, "bold"), justify= LEFT)
    el_dado.place(x=160,y=350)
    prepareWindow()
    prepareGame()
